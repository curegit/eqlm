...
from .core import NamedStencil, laplacian_sharpening
...
def laps(*, input_file: Path | str | bytes | None, output_file: Path | str | AutoUniquePath | BufferedIOBase | None, stencil: NamedStencil, coef: float = 1.0, include_alpha: bool = False, gamma: float | None = None, deep: bool = False, slow: bool = False, orientation: bool = True) -> int:
    exit_code = 0

    x, icc = import_image(input_file, normalize=True, orientation=orientation)

    bgr, alpha = split_alpha(x)
    f, g = color_transforms(Color.RGB, gamma=gamma, transpose=False)
    a = f(bgr)
    b = laplacian_sharpening(a, stencil.value, coef=coef, clip=(0.0, 1.0))
    if alpha is not None and include_alpha:
        alpha = laplacian_sharpening(alpha, stencil.value,coef=coef, clip=(0.0, 1.0))
